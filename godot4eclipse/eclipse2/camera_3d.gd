...
func _input(event):
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_MIDDLE:  # Godot 4'te MOUSE_BUTTON_MIDDLE yerine BUTTON_MIDDLE kullanılır
			mouse_middle_pressed = event.pressed
			last_mouse_position = event.position
		elif event.button_index == MOUSE_BUTTON_LEFT:  # Sol tıklama için BUTTON_LEFT kullanılır
