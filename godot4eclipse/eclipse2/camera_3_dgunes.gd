...
func _ready():
	# Dünya düğümünü bul
	earth = get_node("/root/Node/Node3Dgunestutulmasi/EarthOrbit/Earth")  # Sahne yapınıza göre yolu kontrol edin
	if earth:
...
func _input(event):
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_MIDDLE:  # Godot 4 için doğru sabit
			mouse_middle_pressed = event.pressed
			last_mouse_position = event.position
		elif event.button_index == MOUSE_BUTTON_LEFT:  # Sol tıklama için doğru sabit
	...
	if event is InputEventMouseMotion and not is_locked_on_earth:
		if mouse_middle_pressed:  # Pan hareketi
			...
			rotate_object_local(Vector3(1, 0, 0), deg_to_rad(-delta.y * sensitivity))  # Yukarı/aşağı
...
func zoom_camera(amount):
	...
	var new_translation = position + (basis.z * amount)
...
func _process(_delta):
	...
	if is_locked_on_earth and earth:
		if position.distance_to(earth.global_transform.origin) < 0.1:
			print("Kamera Dünya ile aynı pozisyonda, odaklanma atlandı.")
		else:
